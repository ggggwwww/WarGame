...
// 의도적 취약: PHP 스트림 래퍼(php://, filter 체인) 차단 안 함
$target = $inc . '-view.php';
...
if ($out !== '') {
    ...
    echo "<h1>Viewer</h1>";
    echo "<p>Try different templates via ?inc=NAME</p>";
}

// RCE 증거를 감지해 FLAG 노출(의도된 정답 검증)
if (isset($_GET['proof'])) {
    // 사용자가 실행 결과로 정확히 이 토큰을 화면에 뿌리면 FLAG를 보여준다
    if (strpos($out, 'PROOF:RCE_OK_7b3c1') !== false) {
        echo "<hr><p>FLAG: flag{filter_chain_master}</p>";
    }
}
?>
...
<head>
    ...
    <title>The Ninth Star: Problem 09</title>
...
<body>
    ...
    <div id="problem-container">
        <h1>Problem 10</h1>
        <div id="content-area">
            <textarea style="visibility:hidden; width:500px; height:500px;">
                hint
                1. LFI
                2. RCE
                3. PHP Filter Chain
        ...
        <div id="flag-submission">
            <form action="submit_flag.php" method="POST">
                ...
                <input type="hidden" id="problem_num" name="problem_num" value="10">
